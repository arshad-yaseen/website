// Generates the SVG loader components in src/ui/components/dot-matrix.
// Run: bun scripts/generate-loaders.ts && bunx oxfmt src/ui/components/dot-matrix
...
const OUT = join(dirname(fileURLToPath(import.meta.url)), "../src/ui/components/dot-matrix");
const HEADER = "// Generated by scripts/generate-loaders.ts. Do not edit.\n\n";

// The 5 by 5 grid sits on a 56 unit viewBox, one dot every 11 units from an inset of 6.
const dotPosition = (index: number): number => 6 + index * 11;
...
const SPECS: Spec[] = [
  ...
  },
  {
    ...
    dot: (c) => {
      ...
      // Quadrants light clockwise from the top left.
      if (c.row < 2) return { delay: (c.col < 2 ? 0 : 1) * 480 };
      return { delay: (c.col > 2 ? 2 : 3) * 480 };
    },
  },
  {
    id: "compile",
  ...
  },
  {
    ...
    dot: (c) => {
      if (c.col !== 0 && c.col !== 4) return null;
      const offset = c.col === 4 ? 1200 : 0;
      return { delay: c.row * 200 + offset };
    },
  },
  {
    id: "vector-index",
...
const buildStyle = (spec: Spec): string => {
  ...
  for (const [name, frames] of Object.entries(keyframes))
    lines.push(`@keyframes ${multi ? `${id}-k-${name}` : `${id}-k`}{${frames}}`);
...
const buildUses = (spec: Spec): string => {
  ...
  for (const c of COORDS) {
    const x = dotPosition(c.col);
    const y = dotPosition(c.row);
    const d = spec.dot(c);
    if (!d) {
      if (spec.offOpacity)
        lines.push(
...
import type { LoaderProps } from "@/ui/types/loader-props";
